...
// Function to parse an element line into an associative array
function parseElement($line) {
    if (empty(trim($line))) return null;

    $data = [];
    [$name, $attributes] = explode('=', $line, 2);
    $data['name'] = trim($name);

    foreach (explode(',', trim($attributes)) as $attr) {
        [$key, $value] = explode(':', trim($attr), 2);
...
// Function to determine the color of an element based on its atomic number
...
// Function to convert an element into HTML representation
...
function createMendeleievTable() {
    ...
    // Generate the HTML for the periodic table
    $html = <<<HTML
<!DOCTYPE html>
<html>
<head>
...
<body>
    <h1>Մենդելեևի պարբերական աղյուսակը</h1>
    <table>
HTML;
